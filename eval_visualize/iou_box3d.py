# Source: https://github.com/facebookresearch/pytorch3d/blob/main/pytorch3d/ops/iou_box3d.py
...
def box3d_overlap(
    boxes1: torch.Tensor, boxes2: torch.Tensor, eps: float = 1e-4, verbose=False
    ...
    # assign all values in a row which are not equal to the maximum value to zero.
    max_tensor, _ = torch.max(iou, dim=1, keepdim=True)
    mask = torch.eq(iou, max_tensor)
    iou = iou * mask
    
    nonzero_iou = torch.nonzero(iou)

    for i in range(nonzero_iou.shape[0]):
        if verbose == True:
            print(f'GT: {nonzero_iou[i,0]}, Pred: {nonzero_iou[i,1]}, IoU = {iou[nonzero_iou[i,0],nonzero_iou[i,1]]}')
            
        if iou[nonzero_iou[i,0],nonzero_iou[i,1]] < 0.5:
            iou[nonzero_iou[i,0],nonzero_iou[i,1]] = 0
            continue
        
        if iou[nonzero_iou[i,0],nonzero_iou[i,1]] > 1:
            print("iou over 1, ", iou[nonzero_iou[i,0],nonzero_iou[i,1]])
            iou[nonzero_iou[i,0],nonzero_iou[i,1]] = 1
        sum_iou += iou[nonzero_iou[i,0],nonzero_iou[i,1]]


    
    return vol, iou, sum_iou
