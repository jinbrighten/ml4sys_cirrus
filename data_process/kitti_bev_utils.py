...
def makeBEVMap(PointCloud_, boundary, draw_half = True):
    ...
    if draw_half:
        PointCloud[:, 0] = np.int_(np.floor(PointCloud[:, 0] / cnf.DISCRETIZATION)) # show corresponding region for rgb image
    else:
